...
export class INNIL_SHEET {
	...
	static rename_rest_labels = (sheet, html, sheetData) => {
		if (!game.settings.get(MODULE_NAME, "sheetSettings").rename_rest_labels)
			return;
		const SR = html[0].querySelector(
			".sheet-header .attributes a.rest.short-rest"
		);
		const LR = html[0].querySelector(
			".sheet-header .attributes a.rest.long-rest"
		);
	...
	static remove_resources = (sheet, html, sheetData) => {
		if (!game.settings.get(MODULE_NAME, "sheetSettings").remove_resources)
			return;
		const resources = html[0].querySelector(
			"section > form > section > div.tab.attributes.flexrow > section > ul"
		);
	...
	static remove_alignment = (sheet, html, sheetData) => {
		if (!game.settings.get(MODULE_NAME, "sheetSettings").remove_alignment)
			return;
		const AL = html[0].querySelector(
	...
	static disable_initiative_button = (sheet, html, sheetData) => {
		if (
			!game.settings.get(MODULE_NAME, "sheetSettings")
				.disable_initiative_button
		)
			return;
		const initButton = html[0].querySelector(
			".dnd5e.sheet.actor .sheet-header .attributes .attribute.initiative > h4"
		);
		if (initButton) {
	...
	static create_dots = (sheet, html) => {
		const limited_use_dots = !!game.settings.get(
			MODULE_NAME,
			"colorSettings"
		).limited_use_dots;
		const spell_slot_dots = !!game.settings.get(
			MODULE_NAME,
			"colorSettings"
		).spell_slot_dots;
		...
		if (spell_slot_dots) {
			...
			for (const o of options) {
				const max = html[0].querySelector(
					`.spell-max[data-level=${o}]`
				);
				if (!max) continue;
		...
		if (limited_use_dots) {
			const itemUses = sheet.object.items.filter(
				(i) => !!i.hasLimitedUses
			);
			for (const o of itemUses) {
				const { value, max } = o.system.uses;
				if (max === 0) continue;
				const itemHTML = html[0].querySelector(
					`.item[data-item-id='${o.id}']`
				);
				// Skip if item is hidden via filter.
				if (!itemHTML) continue;
				const position =
					o.type === "spell" ? "beforeBegin" : "afterEnd";
				const adjacent =
					o.type === "spell"
				...
				if (o.type !== "spell") {
					...
					dotContainer.innerHTML =
						Array.fromRange(Math.min(10, max)).reduce((acc, e) => {
							if (e < value)
								return acc + `<span class="dot"></span>`;
		...
		if (spell_slot_dots || limited_use_dots) {
			for (let dot of html[0].querySelectorAll(".dot")) {
				dot.addEventListener("click", async (ev) => {
					...
					if (!item) {
						const spellLevel =
							ev.currentTarget.parentElement.outerHTML.match(
								/data-level="(.*?)"/
							)[1];
						if (!!spellLevel) {
							const path = `system.spells.${spellLevel}.value`;
							const { value } = actor.system.spells[spellLevel];
							const diff = ev.currentTarget.classList.contains(
								"empty"
							)
								? 1
								: -1;
	...
	static create_toggle_on_attunement_button = (sheet, html) => {
		html[0].addEventListener("click", (event) => {
			const attunement_icon = event.target?.closest(
	...
	static refreshColors = () => {
		...
		document.documentElement.style.setProperty(
			"--always_prepared_color",
			cap
		);
		document.documentElement.style.setProperty("--color_proficient", prof);
		document.documentElement.style.setProperty(
			"--color_half_proficient",
			half_prof
		);
		document.documentElement.style.setProperty(
			"--color_twice_proficient",
			twice_prof
		);

		// set item rarity colors on sheet.
		const { uncommon, rare, very_rare, legendary, artifact } =
	...
	static pretty_trait_selector = (selector, html, context) => {
		...
		if (
			...
			["system.traits.toolProf", "system.traits.armorProf"].includes(
				selector.attribute
			)
		) {
	...
	static collapsible_headers = (sheet, html) => {
		if (
			!game.settings.get(MODULE_NAME, "sheetSettings").collapsible_headers
		)
			return;

		// get the headers.
		const headers = html[0].querySelectorAll(
			".dnd5e .items-list .items-header h3"
		);
		const bioHeaders = html[0].querySelectorAll(
			".dnd5e.sheet.actor .characteristics label"
		);
		...
		for (let header of headers) {
			...
			const applyNoCreate = !!foundry.utils.getProperty(
				sheet,
				`section-visibility.${header.innerText}`
			);
			...
			header.addEventListener("click", (event) => {
				const currentDisplay = !!foundry.utils.getProperty(
					sheet,
					`section-visibility.${header.innerText}`
				);
				foundry.utils.setProperty(
					sheet,
					`section-visibility.${event.target.innerText}`,
					!currentDisplay
				);
		...
		for (let header of bioHeaders) {
			...
			const collapsed = !!foundry.utils.getProperty(
				sheet,
				`section-visibility.${header.innerText}`
			);
