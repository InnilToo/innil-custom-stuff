...
async function TWILIGHT_SANCTUARY(
  ...
  async function grantTempHP() {
    ...
    const { total } =
      (await item.rollDamage({ options: { fastForward: true } })) ?? {};
    if (!total) return;
    ...
    if (total > temp) {
      ui.notifications.info(
        `Granting temporary hit points to ${target.document.name}!`
      );
...
async function HARNESS_DIVINE_POWER(
  ...
  async function harness(html, event) {
    ...
    return ChatMessage.create({
      speaker,
      content: `${actor.name} recovered a spell slot using ${item.name}.`,
    });
  }
}
...
async function ARCANE_RECOVERY(
  ...
  if (!available) {
    ui.notifications.warn("DND5E.AbilityUseUnavailableHint", {
      localize: true,
    });
    return;
  }
  ...
  // creating the form.
  const levels = Object.entries(actor.system.spells).reduce(
    (acc, [key, values]) => {
      ...
      const slots = Array.fromRange(values.max).reduce((ac, n) => {
        ...
        return (
          ac +
          `<input type="checkbox" data-key="${key}" data-level="${level}" ${cd}>`
        );
      }, "");
      const label =
        key === "pact"
  ...
  async function recover(html, event) {
    ...
    return ChatMessage.create({
      speaker,
      content: `${actor.name} recovered spell slots using ${item.name}`,
    });
  }
}
...
async function EXPERIMENTAL_ELIXIR(
  ...
  const elixirTypes = foundry.utils.expandObject({
    alacrity: {
      ...
      data: {
        ...
        "flags.visual-active-effects.data.intro":
          "<p>You add a <strong>1d8</strong> bonus to initiative rolls for 1 minute.</p>",
      },
    },
    boldness: {
      name: "Boldness",
      data: {
        ...
        "flags.visual-active-effects.data.intro":
          "<p>You add a <strong>1d4</strong> bonus to attack rolls and saving throws for 1 minute.</p>",
      },
    },
    flight: {
      name: "Flight",
      data: {
        ...
        "flags.visual-active-effects.data.intro":
          "<p>You have a flying speed of 10 feet for 10 minutes.</p>",
      },
    },
    healing: {
      name: "Healing",
    ...
    resilience: {
      ...
      data: {
        ...
        "flags.visual-active-effects.data.intro":
          "<p>You add a <strong>+1</strong> bonus to AC for 10 minutes.</p>",
      },
    },
    swiftness: {
      name: "Swiftness",
      data: {
        ...
        "flags.visual-active-effects.data.intro":
          "<p>Your walking speed increases by 10 feet for 1 hour.</p>",
      },
    },
    transformation: {
      name: "Transformation",
      data: {
        ...
        "flags.visual-active-effects.data.intro":
          "<p>The drinker's body is transformed as if by the @UUID[Compendium.innil-catalogs.spells.DRN4UHvQDhzHZ1vD] spell. The drinker determines the transformation caused by the spell, the effects of which last for 10 minutes.</p>",
      },
    },
  });
  const universal = {
  ...
  new Dialog(
      ...
      buttons: {
        experiment: {
          ...
          callback: async () => {
            ...
            return ChatMessage.create({
              speaker,
              content: `${actor.name} created ${keys.length} random single-effect elixirs.`,
            });
          },
        },
        build: {
    ...
    { classes: ["dialog", "elixir"], height: "auto" }
  ).render(true);

  // BUILD DIALOG
  const content = await TextEditor.enrichHTML(
    Object.entries(elixirTypes).reduce((acc, [key, { name, data }]) => {
      const text = foundry.utils.getProperty(
        data,
        "flags.visual-active-effects.data.intro"
      );
      return (
        ...
        `
    <div class="form-group">
      <label for="elixir-${key}"> <strong>${name}.</strong> ${text.slice(
          3,
          text.length - 4
        )}</label>
      <div class="form-fields">
  ...
  const buildDialog = new Dialog(
    {
      ...
      buttons: {
        create: {
          ...
          label: "Create Elixir",
          callback: async (html) => {
            const keys = [...html[0].querySelectorAll("input:checked")].map(
              (i) => i.dataset.key
            );
            ...
            if (!value) {
              ui.notifications.warn(
                "You have no spell slots of the appropriate level."
              );
              return buildDialog.render();
            }
            ...
            return ChatMessage.create({
              ...
              content: `${
                actor.name
              } expended a ${keys.length.ordinalString()} level spell slot and created an elixir.`,
            });
          },
        },
      },
  ...
  function elixirBuilder(keys = []) {
    ...
    let description = keys.reduce((acc, e) => {
      ...
      const intro = foundry.utils.getProperty(
        data,
        "flags.visual-active-effects.data.intro"
      );
    ...
    const flavor =
      randomFlavor[Math.floor(Math.random() * randomFlavor.length)];
    ...
    return [
      {
        ...
        system: {
          description: {
            value: `<p><em>${flavor}</em></p> <hr> ${description}`,
          },
          weight: 0.5,
          rarity: "common",
  ...
  function createEffects(keys = []) {
    return keys.reduce((acc, key) => {
      ...
      const elixData = foundry.utils.mergeObject(
        elixirTypes[key].data,
        universal
      );
  ...
  async function experimentation() {
    ...
    const { rolls } = await elixRoll.toMessage({
      speaker,
      flavor: `${actor.name} rolls random elixirs`,
    });

    // for each active die, map to an elixir type.
    const elixirKeys = Object.keys(elixirTypes);
    return rolls[0].dice[0].results
      .filter((i) => i.active)
      .map((i) => elixirKeys[i.result - 1]);
  }
}
...
async function STARRY_FORM(
  ...
  const intro = {
    ...
    chalice:
      "<p>Whenever you cast a spell using a spell slot that restores hit points to a creature, you or another creature within 30 feet of you can regain hit points equal to 1d8 + your Wisdom modifier.</p>",
    dragon:
...
async function FONT_OF_MAGIC(
  ...
  const hasAvailableSorceryPoints =
