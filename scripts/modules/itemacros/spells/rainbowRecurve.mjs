...
export async function RAINBOW_RECURVE(
  item,
  speaker,
  actor,
  token,
  ...
  if (!ItemMacroHelpers._getDependencies(DEPEND.CN, DEPEND.RG))
    return item.use();
  ...
  async function chooseArrow() {
    ...
    const content =
      available.reduce((acc, { color, text }) => {
        return (
          acc +
          `
    ...
    d = new Dialog(
      {
        ...
        render: (html) => {
          html[0]
            ...
            .forEach((n) => n.addEventListener("click", shootArrow));
        },
      },
  ...
  async function shootArrow(event) {
    ...
    if (addSave) {
      ...
      newSaveButton.innerHTML = game.i18n.format("CN.ButtonSavingThrow", {
        dc,
        saveType,
      });
