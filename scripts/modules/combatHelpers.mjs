...
export class CombatEnhancements {
  ...
  static _spendReaction(item) {
    ...
    const has = item.actor.effects.find(
      (e) => e.flags.core?.statusId === "reaction"
    );
    if (has) return;
    const combatant =
      item.actor.token?.combatant ?? item.actor.getActiveTokens()[0]?.combatant;
    if (!combatant) return;
    ...
    reaction.flags[DEPEND.VAE].data = {
      intro:
        "<p>" +
        game.i18n.format("INNIL.StatusConditionReactionDescription", {
          name: item.name,
        }) +
        "</p>",
