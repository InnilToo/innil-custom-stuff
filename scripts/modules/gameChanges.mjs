...
function _moveItemToSharedInventory(item, array) {
  if (
    !["weapon", "equipment", "consumable", "tool", "backpack", "loot"].includes(
      item.type
    )
  )
    return;
...
export async function _itemStatusCondition(sheet, html) {
  ...
  const options = CONFIG.statusEffects
    .filter((s) => {
      return !sheet.document.effects.find(
        (e) => e.flags.core?.statusId === s.id
      );
    })
    .sort((a, b) => a.label.localeCompare(b.label))
    .reduce(function (acc, s) {
      return (
        acc + `<option value="${s.id}">${game.i18n.localize(s.label)}</option>`
      );
    }, "");
  ...
  if (add)
    add.addEventListener("click", async function () {
      const id =
        sheet.document.uuid.replaceAll(".", "-") + "-" + "add-status-condition";
