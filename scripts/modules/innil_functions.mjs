 ...
 */
export async function _teleportTokens(
  crosshairsConfig = {},
  fade = true,
  fadeDuration = 500
) {
  ...
  const update = await canvas.scene.updateEmbeddedDocuments("Token", updates, {
    animate: false,
  });
  ...
  return new Dialog(
    {
      ...
      render: (html) => {
        html[0].querySelectorAll(".innil-whisper .player-name").forEach((n) => {
...
export class EXHAUSTION {
  ...
  static async updateExhaustion(num, actor) {
    ...
    const data = {
      label: game.i18n.localize("INNIL.StatusConditionExhaustion"),
      "flags.core.statusId": "exhaustion",
      "flags.visual-active-effects.data.intro": `<p>${game.i18n.format(
