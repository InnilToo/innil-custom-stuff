...
export class INNIL_REST {
	static toggleLR = async (bool) => {
		...
		const currentValue =
			bool === undefined
				? game.settings.get(MODULE_NAME, "toggleLR")
				: !bool;
	...
	static toggleSR = async (bool) => {
		...
		const currentValue =
			bool === undefined
				? game.settings.get(MODULE_NAME, "toggleSR")
				: !bool;
...
export class INNIL_CATALOG {
	...
	static spawnFromCatalog = async (
		...
		const updates = {
			actor: foundry.utils.mergeObject(
				updatesActor,
				warpgateObjects.updates?.actor ?? {}
			),
			token: foundry.utils.mergeObject(
				updatesToken,
		...
		const callbackPre = async (loc, updates) => {
			...
			const provided = foundry.utils.getProperty(
				warpgateObjects,
				"updates.token.img"
			);
			if (!!provided) {
	...
	static mutateFromCatalog = async (
		actorName,
		catalog = "monsters",
		warpgateObjects = {}
	) => {
		...
		for (let item of tokenDoc.actor.toObject().items)
			updatesItems[item.name] = warpgate.CONST.DELETE;

		// handle effects:
		const updatesEffects = {};
		for (let effect of updatesActor.effects)
			updatesEffects[effect.label] = effect;
		for (let effect of tokenDoc.actor.effects) {
			if (!effect.isTemporary)
				updatesEffects[effect.label] = warpgate.CONST.DELETE;
		}
...
export class INNIL_UTILS {
	...
	static teleportTokens = async (
		size = 4,
		{ fade = true, fadeDuration = 500, clearTargets = true } = {}
	) => {
		// pick area of tokens.
	...
	static apply_damage = async (
		actor,
		damages,
		{ showrolls = false, globalModifier = 1 } = {}
	) => {
		...
		const damageArray =
			typeof damages === "string" ? [[damages, ""]] : damages;
	...
	static whisper_players = () => {
		...
		const characterIds = users
			.map((u) => u.character?.id)
			.filter((i) => !!i);
		const selectedPlayerIds = canvas.tokens.controlled
			.map((i) => i.actor.id)
			.filter((i) => characterIds.includes(i));
		const options =
			users.reduce((acc, { id, name, character }) => {
				const checked =
					!!character && selectedPlayerIds.includes(character.id)
						? "selected"
						: "";
				return (
					acc +
					`<span class="whisper-dialog-player-name ${checked}" id="${id}">${name}</span>`
				);
			}, `<form><div class="form-fields whisper-dialog">`) +
			`</div></form>`;
		...
		return new Dialog({
			...
			buttons: {
				go: {
					...
					label: "Whisper",
					callback: async (html) => {
						let content = html[0].querySelector(
							"textarea[id=message]"
						).value;
						if (!content) return;
			...
			render: (html) => {
				...
				for (let playerName of html[0].querySelectorAll(
	...
	static increase_exhaustion = async (actor) => {
		...
		const exhaustion = actor.effects.find(
			(i) => i.getFlag("core", "statusId") === "exhaustion"
		);

		// if exhausted, increase the level.
		if (!!exhaustion) {
			const currentLevel = exhaustion.getFlag(
				"innil-custom-stuff",
				"exhaustion"
			);
	...
	static update_exhaustion = async (num, actor) => {
		...
		let exhaustion = actor.effects.find(
			(i) => i.getFlag("core", "statusId") === "exhaustion"
		);
		...
		if (num === 6) {
			...
			const dead = foundry.utils.duplicate(
				CONFIG.statusEffects.find((i) => i.id === "dead")
			);
