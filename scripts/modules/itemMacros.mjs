  ...
  static _constructDetectionModeEffectData({ modes = [], item }) {
    const onCreate = async function () {
      ...
      return token.document.update({
        detectionModes: previousModes.filter((m) => m.id !== "basicSight"),
      });
    };
...
export class ItemMacro extends MacroConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "modules/innil-custom-stuff/templates/itemacro-config.hbs",
  ...
  /** @override */
  async getData() {
    const options = Object.keys(INNIL.ITEMACRO).map((key) => ({ value: key, label: key }));
  ...
  static hasMacro() {
    ...
    if (data.api in INNIL.ITEMACRO) return true;
  ...
  static getMacro() {
    const data = this.flags[MODULE]?.itemacro;
    let body;
    if (data.api in INNIL.ITEMACRO) body = `return INNIL.ITEMACRO.${data.api}(...arguments);`;
