...
export class INNIL_SOCKETS {
  ...
  static loadTextureForAllSocketOn() {
    game.socket.on(`world.${game.world.id}`, (request) => {
      if (request.action === "loadTextureForAll") {
        INNIL_SOCKETS.loadTextureForAll(request.data.src, false);
      }
    });
  }
  ...
  static createTilesSocketOn() {
    game.socket.on(`world.${game.world.id}`, (request) => {
      if (request.action === "createTiles") {
        INNIL_SOCKETS.createTiles(request.data.tileData, false);
      }
    });
  }
  ...
  static awardLootSocketOn() {
    game.socket.on(`world.${game.world.id}`, (request) => {
      if (request.action === "awardLoot") {
        INNIL_SOCKETS.awardLoot(request.data.backpackUuid, false);
      }
    });
  }
  ...
  static updateTokensSocketOn() {
    game.socket.on(`world.${game.world.id}`, (request) => {
      if (request.action === "updateToken") {
        INNIL_SOCKETS.updateTokens(request.data.updates, false);
      }
    });
  }
  ...
  static grantItemsSocketOn() {
    game.socket.on(`world.${game.world.id}`, (request) => {
      if (
        request.action === "grantItems" &&
        request.data.userId === game.user.id
      ) {
        const token = canvas.scene.tokens.get(request.data.tokenId);
        return token.actor.createEmbeddedDocuments(
          "Item",
  ...
  static async _onDropData(canvas, data) {
    ...
    return INNIL_SOCKETS.grantItems([itemData], tokens[0].id);
  }
}
...
function _getTargetUser(tokenId) {
  ...
  const user =
    game.users.find((u) => {
      return (
        u.active && !u.isGM && canvas.scene.tokens.get(tokenId).actor.isOwner
      );
    }) ??
    game.users.find((u) => {
...
async function _pickTokenTarget(tokens, itemData) {
  ...
  const callback = async function (event, { top, middle, bottom }) {
    ...
    return INNIL_SOCKETS.grantItems([itemData], tokenId);
  };
