...
export class IdentifiersMenu extends SettingsMenu {
  ...
  /** @override */
  async _updateObject(event, formData) {
    formData["monster-catalog-key"] =
      formData["monster-catalog-key"]?.filter((key) => key.trim().length > 0) ??
      [];
  ...
  activateListeners(html) {
    ...
    html[0].addEventListener("click", (e) =>
      e.target
        .closest("[data-action='delete-key']")
        ?.closest(".form-group")
        .remove()
    );
    html[0]
      .querySelectorAll("[data-action='add-key']")
      .forEach((n) => n.addEventListener("click", this._onAddKey.bind(this)));
  }
  ...
  _onAddKey(event) {
    ...
    event.currentTarget
      .closest("form")
      .querySelector(`.keys[data-type='${event.currentTarget.dataset.type}']`)
