...
export class DamageApplicator extends Application {
  ...
  activateListeners(html) {
    ...
    html[0].querySelectorAll("[data-action]").forEach((n) => {
      ...
      else if (action === "pan-to-token")
        n.addEventListener("click", this._onPanToken.bind(this));
      else if (action === "apply-damage")
        n.addEventListener("click", this._onApplyDamage.bind(this));
      else if (action === "render-actor")
        n.addEventListener("click", this._onRenderActor.bind(this));
      else if (action === "apply-damage-all")
        n.addEventListener("click", this._onApplyDamageAll.bind(this));
      else if (action === "saving-throw-all")
        n.addEventListener("click", this._onRollSaveAll.bind(this));
    });
  }
  ...
  static _appendMoreDamageRollData(message) {
    ...
    for (const term of roll.terms) {
      ...
      if (!fl) {
        ...
        else if (damageLabels.includes(fl))
          type = Object.entries(CONFIG.DND5E.damageTypes).find(
            (dt) => dt[1] === fl
          )[0];
