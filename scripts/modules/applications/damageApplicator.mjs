...
export class DamageApplicator extends Application {
  ...
  get targets() {
    const tokens = (
      game.user.isGM ? this.tokens : canvas.tokens.placeables
  ...
  activateListeners(html) {
    ...
    html[0]
      .querySelectorAll("[data-action='toggle-trait']")
      .forEach((n) =>
        n.addEventListener("click", this._onToggleTrait.bind(this))
      );
    html[0]
      .querySelectorAll("[data-action='saving-throw']")
      .forEach((n) => n.addEventListener("click", this._onRollSave.bind(this)));
    html[0]
      .querySelectorAll("[data-action='toggle-throw']")
      .forEach((n) =>
        n.addEventListener("click", this._onToggleSuccess.bind(this))
      );
    html[0]
      .querySelectorAll("[data-action='remove-actor']")
      .forEach((n) =>
        n.addEventListener("click", this._onRemoveActor.bind(this))
      );
    html[0]
      .querySelectorAll("[data-action='pan-to-token']")
      .forEach((n) => n.addEventListener("click", this._onPanToken.bind(this)));
    html[0]
      .querySelectorAll("[data-action='apply-damage']")
      .forEach((n) =>
        n.addEventListener("click", this._onApplyDamage.bind(this))
      );
    html[0]
      .querySelectorAll("[data-action='render-actor']")
      .forEach((n) =>
        n.addEventListener("click", this._onRenderActor.bind(this))
      );
    html[0]
      .querySelector("[data-action='apply-damage-all']")
      .addEventListener("click", this._onApplyDamageAll.bind(this));
    html[0]
      .querySelector("[data-action='saving-throw-all']")
      ?.addEventListener("click", this._onRollSaveAll.bind(this));
  }
  ...
  _onRenderActor(event) {
    return canvas.scene.tokens
      .get(event.currentTarget.closest(".actor").dataset.tokenId)
  ...
  async _onApplyDamage(event) {
    ...
    const values = {};
    for (const input of this.element[0].querySelectorAll(
      ".damage-types .type input"
    )) {
    ...
    const madeSave = actorEl.querySelector(
      "[data-action='toggle-throw'].success"
    );
  ...
  _updateHitPointValues(actor) {
    const hp = canvas.scene.tokens.get(actor.dataset.tokenId).actor.system
      .attributes.hp;
    const [value, temp, max, tempmax] =
      actor.querySelectorAll("[data-attr^='hp-']");
  ...
  async _onApplyDamageAll(event) {
    this.element[0]
      .querySelectorAll(".actor [data-action='apply-damage']")
      .forEach((n) => n.dispatchEvent(new PointerEvent("click", event)));
  }
  ...
  async _onRollSaveAll(event) {
    this.element[0]
      .querySelectorAll(".actor [data-action='saving-throw']")
      .forEach((n) => {
  ...
  async _onPanToken(event) {
    const app = event.currentTarget.closest(
      ".innil-custom-stuff.damage-applicator"
    );
    app.classList.toggle("fade", true);
    const { x, y } = canvas.tokens.get(
      event.currentTarget.closest(".actor").dataset.tokenId
    ).center;
  ...
  async _onRollSave(event) {
    ...
    const token = canvas.scene.tokens.get(
      target.closest(".actor").dataset.tokenId
    );
    const data = target.dataset;
    const roll = await token.actor.rollAbilitySave(data.ability, {
      event,
      targetValue: data.dc,
    });
    if (!roll) {
    ...
    const res = target
      .closest(".util")
      .querySelector("[data-action='toggle-throw']");
  ...
  _getActorDamageTraits(actor) {
    return ["dr", "di", "dv"].reduce(
      (acc, d) => {
        const trait = actor.system.traits[d];
        const types = new Set(trait.value);
        const bypasses = trait.bypasses.filter((b) =>
          this.bypasses.includes(b)
        ); // the bypasses that matter.
        ...
        for (const type of types) {
          ...
          const hasBypass =
            CONFIG.DND5E.physicalDamageTypes[type] && bypasses.size > 0;
          if (hasBypass) {
            for (const b of bypasses) {
              acc[d].push({
                key: type,
                label: `Non-${CONFIG.DND5E.physicalWeaponProperties[b].slice(
                  0,
                  3
                )} ${CONFIG.DND5E.damageTypes[type]}`,
                bypass: b,
  ...
  _getActorHasActivePlayerOwner(actor) {
    return game.users.some((user) => {
      return (
        !user.isGM && user.active && actor.testUserPermission(user, "OWNER")
      );
    });
  }
  ...
  static async _appendToDamageRolls(message, [html]) {
    ...
    div.innerHTML = await renderTemplate(
      "modules/innil-custom-stuff/templates/damageRollButtons.hbs",
      data
    );
    div.querySelectorAll("[data-action]").forEach((n) => {
      ...
      if (action === "render") {
        n.addEventListener("click", (event) =>
          new DamageApplicator({ message }).render(true)
        );
      } else if (action === "quick-apply") {
        n.addEventListener("click", (event) =>
          new DamageApplicator({ message }).damageAll(event, { save: false })
        );
      } else if (action === "save-and-apply") {
        n.addEventListener("click", (event) =>
          new DamageApplicator({ message }).damageAll(event, { save: true })
        );
  ...
  static _appendDamageRollData(item, config) {
    ...
    const bypasses = item.system.properties
      ? Object.keys(CONFIG.DND5E.physicalWeaponProperties).filter((p) => {
