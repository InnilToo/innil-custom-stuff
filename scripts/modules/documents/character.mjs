  ...
  static init() {
    ...
    CONFIG.Actor.documentClass = class Actor5e extends (
      ...
      static computeClassProgression(
        ...
        super.computeClassProgression(progression, cls, {
          actor,
          spellcasting,
          count,
        });
      ...
      static computeWardenProgression(
        progression,
        actor,
        cls,
        spellcasting,
      ...
      static prepareSpellcastingSlots(
        ...
        if (type === "warden")
          this.prepareWardenSlots(spells, actor, progression);
      }

      static prepareWardenSlots(spells, actor, progression) {
        const override = Number.isNumeric(spells.pact.override)
          ? Math.max(parseInt(spells.pact.override), 0)
          : 0;
        if (override > 0) return;

        const level = Number.isNumeric(progression.warden)
          ? parseInt(progression.warden)
          : 0;
        if (level > 0) {
          const slots =
            CONFIG.DND5E.spellcastingTypes.warden.progression[level];
          spells.pact.max += slots.max;
          spells.pact.level = Math.max(spells.pact.level, slots.level);
          spells.pact.value = Math.clamped(
            spells.pact.value,
            0,
            spells.pact.max
          );
