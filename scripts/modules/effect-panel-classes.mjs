...
export class EffectsPanelApp extends Application {
	static get defaultOptions(){
		return foundry.utils.mergeObject(super.defaultOptions, {
			id: "effects-panel",
			popOut: false,
			template: "modules/innil-custom-stuff/templates/effects-panel.html"
		});
	}
    ...
    async getData(options){
        ...
        for(let eff of this._controller.data.disabledEffects){
            const desc = foundry.utils.getProperty(eff, "flags.convenientDescription");
            if(!!desc) await eff.document.update({"flags.convenientDescription": await TextEditor.enrichHTML(desc, {async: true})});
        }
...
export class INNIL_EFFECTS_PANEL {
