...
export class DM_TOOL {
  ...
  static async deleteConditionsFromTokens(tokens, statusIds) {
    return tokens.map((token) =>
      token.actor.deleteEmbeddedDocuments(
        "ActiveEffect",
        this.getDeleteIds(token, statusIds)
      )
    );
  }
  ...
  static async applyConditionsToTokens(tokens, conditionDatas) {
    return tokens.map((token) => {
      const conditions = conditionDatas.filter((c) => {
        return !token.actor.effects.find(
          (eff) => eff.flags.core?.statusId === c.flags.core.statusId
        );
      });
  ...
  static async requestSavingThrowsFromTokens(
    tokens,
    { ability, targetValue, event }
  ) {
    const results = [];
    for (const token of tokens) {
      const save = await token.actor.rollAbilitySave(ability, {
        event,
        targetValue,
      });
      if (!save) continue;
  ...
  static async MAIN_FUNCTION({
    statusCondition = {},
    damage = {},
    savingThrow = {},
  }) {
    const tokens = canvas.tokens.controlled.filter(
      (i) => !i.actor.hasPlayerOwner
    );
    ...
    if (saveNeeded) {
      const saves = await this.requestSavingThrowsFromTokens(
        tokens,
        savingThrow
      );
      const failedTokens = saves
        .filter((save) => !save.success)
        .map((save) => save.token);
      const succeedTokens = saves
        .filter((save) => save.success)
        .map((save) => save.token);
  ...
  static createDamageOrEffectRow(event) {
    const target = event.target.closest(
      "[name='status-create'], [name='damage-create']"
    );
    if (!target) return;
    ...
    if (target.name === "status-create") {
      container = target
        .closest("div.dialog-content")
        .querySelector("div.status-container");
      const has = [
        ...container.querySelectorAll("[name='status-effects']"),
      ].map((n) => n.value);
      const effectOptions = CONFIG.statusEffects
        .sort((a, b) => {
        ...
        .reduce((acc, { id, label }) => {
          ...
          return (
            acc + `<option value="${id}">${game.i18n.localize(label)}</option>`
          );
        }, "");
      ...
      container = target
        .closest("div.dialog-content")
        .querySelector("div.damage-container");
      const damageOptions = Object.entries(CONFIG.DND5E.damageTypes)
        .filter(([type]) => {
          return !(type in CONFIG.DND5E.physicalDamageTypes);
        })
        .concat(Object.entries(CONFIG.DND5E.physicalDamageTypes))
  ...
  static async RENDER() {
    ...
    const abilityOptions = Object.entries(CONFIG.DND5E.abilities).reduce(
      (acc, [key, label]) => {
        return acc + `<option value="${key}">${label}</option>`;
      },
      "<option value=''>&mdash;</option>"
    );
    const template = "modules/innil-custom-stuff/templates/dm_tool.hbs";
    ...
    const d = new Dialog(
      {
        ...
        buttons: {
          ...
          remove: {
            ...
            callback: async (html) => {
              const statusIds = [
                ...html[0].querySelectorAll("[name='status-effects']"),
              ].map((i) => i.value);
          ...
          removeAll: {
            ...
            callback: async () => {
              tokens.map((token) => {
                ...
                return token.actor.deleteEmbeddedDocuments(
                  "ActiveEffect",
                  deleteIds
                );
              });
        ...
        render: (html) => {
          html[0].addEventListener("click", (event) => {
            ...
            event.target
              .closest("[name='status-delete'], [name='damage-delete']")
  ...
  static gatherDamageInputs(html) {
    ...
    const properties = [
      ...html[0].querySelectorAll(".weaponProperties input:checked"),
    ].map((b) => b.id);

    // construct damage parts.
    const damageInputs = html[0].querySelectorAll(
      "div.damage-container div.form-group"
    );
    const parts = [];
  ...
  static gatherEffectInputs(html) {
    ...
    const effectInputs = html[0].querySelectorAll(
      "div.status-container div.form-group"
    );
    const effectData = [];
...
export function _appendDataToDamageRolls(item, config) {
  ...
  config.messageData[`flags.${MODULE}.properties`] = Object.entries(
...
export function _addFlavorListenerToDamageRolls(message, html) {
  ...
  totals = Object.entries(totals).map(([key, value]) => {
    if (key === types[0])
      return [totals[types[0]] + (total - otherSum), types[0]];
    return [value, key];
  });
  console.log({ totals });
...
export function calculateDamageTakenForToken(
  token,
  parts,
  { global = 1, properties = [] } = {}
) {
  ...
  const partsModified = parts.map(([value, type]) => {
    const imm =
      di.has(type) &&
      !properties.some((p) => di.bypasses.has(p) && type in phys);
    const res =
      dr.has(type) &&
      !properties.some((p) => dr.bypasses.has(p) && type in phys);
    const vul =
      dv.has(type) &&
      !properties.some((p) => dv.bypasses.has(p) && type in phys);
